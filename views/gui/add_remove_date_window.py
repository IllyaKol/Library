...
class AddRemoveDate(QWidget):
    def __init__(self, library):
        super(AddRemoveDate, self).__init__()
        self.setGeometry(800, 800, 600, 250)
        self.setWindowTitle('Add remove date')
    ...
    def initialize_buttons(self):
        self.remove_date = QLabel('Return date')
        ...
        grid.addWidget(self.remove_date, 1, 0)
    ...
    def save(self):
        if self.check_fields():
            found = True
            for i in self.library.history:
                if self.visitor_id_edit.text() == str(i.visitor_id) and self.book_id_edit.text() == str(
                        i.book_id) and i.returning_date is None:
                    found = False
            if found == True:
                QMessageBox.question(self, 'Message', "This user do not take this book", QMessageBox.Ok)
                return
            #
            # found = True
            ...
            self.library.add_remove_date(self.visitor_id_edit.text(), self.book_id_edit.text(), self.dateEdit.text())
            self.show_window_ok()

    def show_window_ok(self):
        button_reply = QMessageBox.question(self, 'Message', "Book was returned", QMessageBox.Ok)
